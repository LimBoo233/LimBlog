...
`ClipTranstion` 还有对应的 so 资产：`TransitionAssets`。
...
Transition Library 是 Animancer 中的一种资产，用于统一管理、调配各种动画的过渡时间。`AnimancerComponet` 会使用 Transition Library 覆盖 transition clip 本身那个统一的 fade duration。 
