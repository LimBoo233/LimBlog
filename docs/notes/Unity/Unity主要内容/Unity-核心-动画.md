...
> [!WARNING]
> 由于底层 Playables API 的问题，即使你使用了多个层，Unity 也始终只会给`OnAninamtorIK()`传入`layerIndex = 0`。
...
此外还可以无视 `Conditions` ，通过代码直接播放动画片段，这是更解耦的作法。完全使用代码播放时，就不需要在 Animator 中连线了。
...
一个 `.fbx` 可能包含多个动画剪辑。可以在 Clips 列表中新增，删除，预览或编辑对应的动画剪辑。你也可以在 Clips 列表添加新的 Clip，然后再下方修改 Start, End 的值和 Clip 名字来切分动画，以将一个 Clip 用作多个 Clips，但注意这种方式创建的 Clip 下方的参数（ex: Loop Time）等都需要重新设置。

![动画选择列表](images/动画选择列表.png)

此处的 `Start` 和 `End` 属性标记了动画的起始和结束帧数。
...
- `Bake Into Pose`：不应用根运动，将动画变为原地运动。
- `Based Upon`：计算旋转时的参考系或基准。
   - `Original`：使用动画文件中存储的原始旋转数据。这是最常用、最直观的选项。
   - `Body Orientation`：Humanoid 角色限定。Unity 会根据角色的上下半身朝向，计算出一个强制把角色躯干（胸口）的视觉朝向，定义为这段动画的正前方。
   - `Root Node Rotation`：仅适用于通用（Generic）角色。使用在 Avatar 中被指定为根节点的那个骨骼的朝向作为基准。
- `Offset`：额外增加一个固定的旋转角度。

> [!NOTE] 根运动 Root Motion
> 根运动，即动画本身记录的位移和旋转。应用根运动时会采用动画中的存储的相关数据，无需手动控制。

**Root Transform Position (Y)**

控制角色在垂直方向的运动。
- `Bake Into Pose`：不应用根运动，将动画变为原地运动。
- `Based Upon (at Start)`：计算 Y 轴位置的参考基准。
...
- `Bake Into Pos`：不应用根运动，将动画变为原地运动。
   ::: info
   勾选此项可以完美解决角色移动时最常见的问题——脚底打滑(Foot Sliding)。
   :::

-  `Based Upon (At Start)`：计算 XZ 轴位置的参考基准。
...
> [!NOTE]
> 此篇介绍的是 Unity 最原生、最~~原始~~老牌的 IK 系统。Unity 已推出的 Unity Animation Rigging Package 把 IK 变成了组件，不再需要大量代码参与。
...
`OnAnimatorMove()` 是 Unity 动画系统中一个回调函数，用来接管和修改角色的根运动 (Root Motion)。Unity 会根据动画文件计算出本帧的移动距离，在使用`OnAnimatorMove()`时，你应该利用这些计算数据来位移：
...
void OnAnimatorMove()
{
	...
	// 通过 CharacterController 组件移动，如此才有碰撞检测
...
`animator.ApplyBuiltinRootMotion()` 方法会应用动画本身的根运动。
...
和 `OnAnimatorIK(int layerIndex)` 一样，它也是在 `Update()` 之后 `LateUpdate()` 之前被调用，并且也是在每帧的状态机和动画处理完成后调用。该回调就会自动应用 `Apply Root Motion`。其中，`OnAnimatorIK(int layerIndex)` 的调用时机更靠前。
...
- `MatchTargetWeightMask weightMask`：权重遮罩。一个非常重要的参数，它告诉系统你关心匹配的哪些部分。
   ...
   - `positionWeight`：一个 `Vector3`，可以分别控制 X, Y, Z 三个轴的位置匹配权重 (0-1)。比如 `new Vector3(1, 1, 0)` 表示只匹配 X 和 Y 轴位置到 `matchPosition`，忽略 Z 轴。
...
它的本质是一个 ScriptableObject，当其挂载到一个 state 时，作为一种资产文件。所有使用相同 Animator 的 GameObject 里的同一个 State，共同读写同一个 `StateMachineBehaviour`。所以不要让 `StateMachineBehaviour` 存任何运行时变量。

类似 `MonoBehaviour`，该脚本可以附加在 `Animator Controller` 的某个特定状态或子状态机上，它的生命周期与该状态的激活状态绑定。但因为它只是一个资产文件，你不能真把它当 `MonoBehaviour` 去用。
...
```csharp
[System.Serializable]  
public struct AudioPoint  
{  
    [Tooltip("音效资源")]   
public AudioClip clip;  
  
    [Range(0f, 1f), Tooltip("动画播放进度的百分比 (0-1)")]  
    public float timeThreshold;  
}  
  
/// <summary>  
/// 用于在动画状态机中指定特定时间点播放音效  
/// </summary>  
public class AudioTriggerBehaviour : UnityEngine.StateMachineBehaviour  
{  
    [Header("音效触发点列表，务必按时间顺序排列")]  
    public List<AudioPoint> audioPoints = new();  
  
    private int _nextIndex;  
    
    // 进入状态时重置所有标记  
    public override void OnStateEnter(Animator animator, 
    AnimatorStateInfo stateInfo,  int layerIndex) => _nextIndex = 0;  
  
	public override void OnStateUpdate(UnityEngine.Animator animator, 
		AnimatorStateInfo stateInfo,int layerIndex)  
    {  
       // 只有在非过渡状态，或者过渡进度较小时才触发，防止融合时双重响声  
       if (animator.IsInTransition(layerIndex) 
	       && stateInfo.normalizedTime > 0.5f) return;  
  
       // 处理循环动画  
       var progress = stateInfo.normalizedTime % 1f;  
  
       // 使用 while 是为了防止动画帧率极低时，一帧跨过了多个音效点  
       while (_nextIndex < audioPoints.Count 
	       && progress >= audioPoints[_nextIndex].timeThreshold)  
       {  
          var point = audioPoints[_nextIndex];  
          SoundManager.MainInstance
	          .PlayEffectSound(point.clip, animator.transform.position);  
...
`animator.GetBehaviour<T>()` 方法，会遍历所有 Layer 和 State 寻找脚本实例：

```cs
var climbBehaviour = _animator.GetBehaviour<ClimbOverBehaviour>();
```
## 状态机的复用
...
属性说明：
- `Slope Limit`（坡度限制）： 角色能够走上去的最大斜坡角度。

- `Step Offset`（台阶高度）：角色能够跨越的最大台阶高度（米）。

- `Skin Width`（皮肤宽度）：胶囊体内部的一个缓冲层厚度。
...
- `Center` / `Radius` / `Height`：共同定义碰撞胶囊体的形状和尺寸。

- `Layer Overrides`（图层覆盖）：更精细地控制碰撞检测。
...
`ChracterController` 的 `isGrounded` 属性用来检测角色是否站在地面上。这个属性只有在调用了 `Move` 方法后才会进行地面检测并修改此属性，且不够靠谱。
...
此外还有一个更简单，用于测试时使用的移动 API `SimpleMove()`。相比 `Move()`，`SimpleMove()` 会自动地、在内部为你施加一个向下的、符合全局物理设置的重力，但它会完全忽略你传入的参数向量的 Y 轴分量，导致不能实现跳跃等功能，只能用来临时玩玩。
