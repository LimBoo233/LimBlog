...
如果希望即使不在 Play 状态下，也能绘制线条，可以在钩子函数`OnDrawGizmos`中绘制线条。
...
// 只有当你在 Hierarchy 面板选中这个物体时，才会画出图形
...
3. Mathf 中的常用方法 —— 一般只计算一次
    ...
    - `Mathf.Atan2`：计算弧度，比 `Atan` 对象限更敏感（$-\pi$, $\pi$）
    - `Mathf,Rad2Deg`：弧度转角度
...
4. 通过点乘判断方位
    ...
    **如何计算两个向量之间的夹角?**
    ...
    // -180 到 180 度
    // 如果目标在右侧，返回正数；在左侧，返回负数
    float signedAngle =
	    Vector3.SignedAngle(forward, directionToTarget, Vector3.up);
    ```
...
8. 平滑阻尼
	...
	- **`current`**: 物体当前的坐标。
	- **`target`**: 你想要到达的目标坐标。    
	- **`ref currentVelocity`**:你需要声明一个私有变量来存储当前速度。`ref` 关键字允许方法在每一帧修改这个值，以便下一帧继续使用；并且方法会根据 `currentVelocity` 来推算这一帧需要的速度。
	- **`smoothTime`**: 到达目标的大致时间。数值越小，移动越快；数值越大，延迟感越强。
	- **`maxSpeed`**: 可选参数，限制物体的最大移动速度。
...
这种方法类似于上面的几种，区别在在于：
- **`CheckSphere`**: 只告诉你有没有，返回`bool`。**性能极高**，因为它一旦扫到一个物体就立即停止计算。
- **`OverlapSphere`**: 告诉你有哪些，返回`Collider[]`数组。**性能开销较大**，因为它涉及内存分配（GC 压力）。

如果你只是想判断能否跳跃，用`CheckSphere`；如果你想给范围内的所有敌人扣血，用 `OverlapSphere`。
## 射线检测
...
当射线从一个 collider 内部发出时，会忽略所在 collider。
...
`Physics.Linecast`检测从起点到终点之间的路径上是否有碰撞体。

| 特性   | Physics.Linecast | Physics.Raycast          |
| ---- | ---------------- | ------------------------ |
| 输入参数 | 两个点 (A 到 B)      | 起点 + 方向 + 长度             |
| 距离控制 | 隐式（由两点间距决定）      | 显式（由 `maxDistance` 参数决定） |
| 思维模型 | **“这两点之间有东西吗？”** | **“往这个方向看，会撞到什么？”**      |
| 性能   | 略高（计算开销极小）       | 极高（计算开销极小）               |

ex：检查玩家和相机目标点之间是否有墙

```cs
if (Physics.Linecast(player.position, cameraTarget.position, out RaycastHit hit))
{
