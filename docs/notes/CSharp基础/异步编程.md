...
3. 善后
	...
	```
::: tip
`cancellationToken.Register()` 后注册的后先调用。
:::

#### `TaskCanceledException` & `OperationCanceledException`

`OperationCanceledException` 其实是 `TaskCanceledException` 的父类。`Task` 位于命名空间 `System.Threading.Tasks`，而 `OperationCanceledException` 位于 `System.Threading` 中。
