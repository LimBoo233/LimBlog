...
找到 codex 配置文件（ex: `C:\Users\Megumin\.codex\config.toml`），在里面填入要的 MCP server。当然也可让 codex 代劳。

可通过 `/mcp` 指令查看已有 MCP 配置。

## 创建简单命令

在 codex 安装目录下创建 `prompts` 文件夹，在其中创建 `.md` 文件并编写提示词，文件名即为命令名。
