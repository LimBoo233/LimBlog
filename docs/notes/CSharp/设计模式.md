...
此示例中使用了`List<Delegate>`来存储所有事件而非使用多播委托，因为多播委托在底层使用了不可变的数组，在频繁删减的情况下性能较差。
...
但是需要注意，事件总线可能会带来的广播风暴问题：一个玩家对攻击了一个怪物，向事件总线推送事件，所有的怪物都需要判断挨打的是不是自己，非常消耗性能。也有一些解决方案，例如 key 不再只是事件类型，或者使用传统的观察者模式，或简单的获取对方引用再调用相应的方法等等。
