...
- 变换矩阵：View Matrix，它移动和旋转整个世界，使摄像机刚好落在 $(0,0,0)$ 并对准Z轴。你的摄像机不会动，而是整个世界在动。
...
flowchart TD
    ...
    subgraph C [阶段II: 定义对象]
        C1[用图元描述对象<br>点/线/三角形]
        C2[定义顶点属性<br>颜色/材质/纹理坐标]
    end
    
    C --> D[阶段III: 几何操作]
    
    subgraph D [阶段III: 几何操作]
        D1[应用模型/视图/投影矩阵]
        D2[执行透视除法]
        D3[应用视口变换]
        D4[变换法向量]
