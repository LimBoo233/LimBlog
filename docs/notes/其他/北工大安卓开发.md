...
未完成的后续内容可以参考某高手的，已经完成的内容也可以做个互补：[某个个人博客里 GitHub 连接指的是 Vitepress 而不是自己 profile 的神人的笔记](https://blog-snowy-theta-58.vercel.app/notes/Mobile%20Computing.html#content-provider)
...
3. Libraries and Android Runtime (库和安卓运行时)
    ...
    不过，ART 运行的仍是 `.dex` 文件，即 Dalvik Executable 格式的字节码文件。
    :::
...
1.  `AndroidManifest.xml` (清单文件)
    ...
    - 指定启动 Activity ：通过 `<intent-filter>` 里的标签 `android.intent.action.MAIN`，指定哪个 Activity 是点APP 图标时第一个打开的。
...
| `onPause()` | Activity 失去前台焦点时调用 (比如来了个弹窗)。<br>该方法保证会被调用，适合用来保存持久化数据。 |
| 可见/不可见 | - |
...
- `getPreferences()`：(Activity 专用) 只想给当前 Activity 存一个私有的配置文件。
- `getSharedPreferences(String name, int mode)`：（App 级别）你给它一个名字（比如 `"myPrefs"`），就可以在 App 的任何地方访问这个共享的配置文件——推荐。
...
// 4. 提交更改
editor.commit();  // 或者用 editor.apply()，它在后台异步提交
```

`DataStore` 是 Google 新推出的、用来替代 `SharedPreferences` 的方案，更安全、更高效。
